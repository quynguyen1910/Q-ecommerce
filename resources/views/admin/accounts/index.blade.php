...
@section('main')
<div class="py-3">
    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addAcount">
...
<tr>
    ...
    <td>{{$account->matkhau_tk}}</td>
    <td>{{$account->email}}</td>
    <td>{{$account->ma_nv ?? '...'}}</td>
    <td>{{ isset($account->ho, $account->ten) ? $account->ho . ' ' . $account->ten : '...' }}</td>
    <td>{{$account->created_at}}</td>
    <td>
        <button type="button" class="btn btn-primary edit-user-btn">
            <i class="fa fa-pencil" aria-hidden="true"></i>
        </button>
        <button class="btn btn-danger deleteUser" data-bs-toggle="modal" data-bs-target="#requestForm">X</button>
    </td>
</tr>
            @endforeach
        </tbody>
...
@push('modal')
<div class="modal fade" id="addAcount" tabindex="-1" aria-labelledby="addAcountLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="addAcountLabel">Thêm Thành Viên</h1>
